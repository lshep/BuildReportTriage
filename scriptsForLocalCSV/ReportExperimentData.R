...
for(ver in versions){
    ...
    tbl <- read.table(paste0("https://master.bioconductor.org/checkResults/",ver,"/data-experiment-LATEST/BUILD_STATUS_DB.txt"), comment.char="")
    ...
    # build results table    
    pkg_json <- read_json(paste0("https://master.bioconductor.org/packages/json/",ver,"/data/experiment/packages.json"))
    ...
    # write.table(DF, paste0("ExperimentDataPkgSummary_",sub(ver, pattern=".", replacement="_",fixed=TRUE), ".csv"), row.names=FALSE, sep=",", quote=FALSE)
    # MasterTbl = DF
    ## Skip down to Make Date spreadsheets
    
    OldTbl <- read.csv(paste0("ExperimentDataPkgSummary_",sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), header=TRUE)
    ...
    write.table(MasterTbl, paste0("ExperimentDataPkgSummary_",sub(ver, pattern=".", replacement="_",fixed=TRUE), ".csv"), row.names=FALSE, sep=",", quote=FALSE)    
    ...
    # first run only to have starting file
    #write.table(colDF, file = paste0("ExperimentDataPkgAcrossAll_",sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), quote=FALSE, sep=",", row.names=FALSE)
    
    r <- read.table(file = paste0("ExperimentDataPkgAcrossAll_",sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), sep=",", header=TRUE, stringsAsFactors=FALSE)
    ...
    write.table(r, file = paste0("ExperimentDataPkgAcrossAll_",sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), quote=FALSE, sep=",", row.names=FALSE)
  ...
  if(perBuilder){
    ...
    for(ab in active_builders){
        ...
        colnames(colDF)[2] = format(Sys.time(), "%b%d")
        # first run only to have starting file
        #write.table(colDF, file = paste0("ExperimentDataPkg_",ab,"_", sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), quote=FALSE, sep=",", row.names=FALSE)
        
        r <- read.table(file = paste0("ExperimentDataPkg_",ab,"_", sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), sep=",", header=TRUE, stringsAsFactors=FALSE)
        idx = match(setdiff(as.character(colDF$pkgs), as.character(r$pkgs)), as.character(colDF$pkgs))
        if(length(idx)!= 0){
            subAdd = colDF[idx,]
        ...
        r = cbind(r, newCol = tempVec)
        colnames(r)[dim(r)[2]] = format(Sys.time(), "%b%d")
        write.table(r, file = paste0("ExperimentDataPkg_",ab,"_", sub(ver, pattern=".", replacement="_",fixed=TRUE),".csv"), quote=FALSE, sep=",", row.names=FALSE)
...
if (length(versions) == 2){
    ...
    builder1 =  read.table(file =
        paste0("ExperimentDataPkgAcrossAll_",sub(versions[1], pattern=".",
                                                 replacement="_",fixed=TRUE),".csv"),
        sep=",", header=TRUE, stringsAsFactors=FALSE)

    builder2 =  read.table(file =
        paste0("ExperimentDataPkgAcrossAll_",sub(versions[2], pattern=".",
                                                 replacement="_",fixed=TRUE),".csv"),
        sep=",", header=TRUE, stringsAsFactors=FALSE)
    ...
    if (!(length(Both)==0)){
        ...
        colnames(colDF)[2] = format(Sys.time(), "%b%d")
    ## first run only to have starting file
    #write.table(colDF, file = "ExperimentDataPkg_BothReleases.csv", quote=FALSE, sep=",", row.names=FALSE)
    
        r <- read.table(file = "ExperimentDataPkg_BothReleases.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
        idx = match(setdiff(as.character(colDF$pkgs), as.character(r$pkgs)), as.character(colDF$pkgs))
        if(length(idx)!= 0){
            subAdd = colDF[idx,]
        ...
        r = cbind(r, newCol = tempVec)
        colnames(r)[dim(r)[2]] = format(Sys.time(), "%b%d")
        write.table(r, file = "ExperimentDataPkg_BothReleases.csv", quote=FALSE, sep=",", row.names=FALSE)
    }else{

        r <- read.table(file = "ExperimentDataPkg_BothReleases.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
        colDF = matrix(rep(0, dim(r)[1]), ncol=1)
        colnames(colDF) = format(Sys.time(), "%b%d")
        r = cbind(r, colDF)
        write.table(r, file = "ExperimentDataPkg_BothReleases.csv", quote=FALSE, sep=",", row.names=FALSE)
