...
both <- read.table(file = "ExperimentDataPkg_BothReleases.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
rel <- read.table(file = "ExperimentDataPkgSummary_3_15.csv", sep=",", header=TRUE,
                  stringsAsFactors=FALSE)
dev <- read.table(file = "ExperimentDataPkgSummary_3_16.csv", sep=",", header=TRUE,
                  stringsAsFactors=FALSE)
...
if(length(RelOnly) > 0){
    ...
    ## first run only
    # write.table(colDF, file = "ExpRelOnly.csv", quote=FALSE, sep=",",row.names=FALSE, na="")
 
    r = read.table(file = "ExpRelOnly.csv", sep=",", header=TRUE,
                   stringsAsFactors=FALSE)
    ...
    write.table(r, file = "ExpRelOnly.csv", quote=FALSE, sep=",",
                row.names=FALSE, na="")
}
...
if(length(DevOnly) > 0){
    ...
    colnames(colDF)[2] = format(Sys.time(), "%b%d")
    ## first run only
    # write.table(colDF, file = "ExpDevOnly.csv", quote=FALSE, sep=",",row.names=FALSE, na="")

    r = read.table(file = "ExpDevOnly.csv", sep=",", header=TRUE,
                   stringsAsFactors=FALSE)
    ...
    r = cbind(r, newCol = tempVec)
    colnames(r)[dim(r)[2]] = format(Sys.time(), "%b%d")
    write.table(r, file = "ExpDevOnly.csv", quote=FALSE, sep=",",
                row.names=FALSE, na="")
}
